...
Proof.
  intros [f1] [f2]. refine (existT _ (fun x => f1 (f2 x)) (Build_Isometric _ (fun x => (inverse f2) (inverse f1 x)) _ _ _)); intros.
...
Global Instance iso_gunit: GrUnit Isometry. Proof. refine (existT _ (fun x => x) (Build_Isometric _ (fun x => x) _ _ _)); intros; auto. Defined.

Global Instance iso_neg: Negate Isometry.
Proof.
  intros [f].
  refine (existT _ (inverse f) (Build_Isometric _ f _ _ _)); assert (forall x : Point, inverse f (f x) == x) by (intros; apply (injective f), surjective); intros; auto.
  - pose proof (surjective f x). pose proof (surjective f y). rewrite <- H0, H1 in H2; assumption.
  - rewrite <- (surjective f x), <- (surjective f y), !H. symmetry. apply distance_preserve.
Defined.

Instance: Setoid Isometry. Proof. constructor; repeat intro; [| rewrite H | rewrite H, H0]; reflexivity. Qed.

Instance: Proper ((=) ==> (=) ==> (=)) iso_binop.
Proof. repeat intro. unfold iso_binop. destruct x, y, x0, y0. unfold equiv, iso_equiv in H, H0. simpl in *. rewrite H0, H. reflexivity. Qed.

Instance: Proper ((=) ==> (=)) iso_neg.
Proof. repeat intro. unfold iso_neg. destruct x, y. unfold equiv, iso_equiv in H. simpl in *. apply (injective x). rewrite surjective, H, surjective. reflexivity. Qed.
